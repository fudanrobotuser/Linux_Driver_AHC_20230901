...
"Usage: eeprog [-fqxdhm] [-16|-8] [ -r addr[:count] | -w addr ]  /dev/i2c-N  i2c-address\n" 
"\n"
"  Address modes: \n"
...
int read_from_eeprom(struct eeprom *e, int addr, int size, int hex)
{
	...
	if (size == 128) {
		eeprom_read_nbyte(e, addr, buf, 128);
...
{
	int c;
	print_info("writing char a to z n");
	for(c = 'a'; c <= 'z'; c++) {
		print_info("%c",c);
...
int main(int argc, char** argv)
{
	...
	while((ret = getopt(argc, argv, "1:8fr:mqhw:xd")) != -1)
	{
		switch(ret)
		{
		case '1':
	...
	die_if(eeprom_open(device, i2c_addr, eeprom_type, highspeed, &e) < 0, 
